...
volatile int flag_vastago_atras = 0;	//Bandera para saber que el vástago esta atrás
volatile int flag_sw2_giro = 0;			//Bandera para cambiar el giro en la interrupción y meterse en antirrebotes_sw2_giro
...
void control_motor(int n_motor, int power, int direccion, int brake){
	switch (n_motor) {
		...
		switch (power){
			case 0:
			OCR1B = 0; break;         //Apagado
			case 1:
			OCR1B = OCR_PWM; break;   //Encendido
			default:
			OCR1B = 0; break;
		}
		...
		switch (power){
			case 0:
			OCR1A = 0; break;         //Apagado
			case 1:
			OCR1A = OCR_PWM; break;   //Encendido 
			default:
			OCR1A = 0; break;
		}
...
void home_lanzador(){
	cont_sw2 = 0;							// Tenemos que poner bandera que nos pasa el grupo del cargador de las bolas a 1, de momento pongo un 1, bandera de motor 1 arriba
	...
	if(cont_sw2 > 1){
		control_motor(2,0,0,0);				// Apago M2
		flag_sw2_derecha = 1;
	}
}
...
//GET_VASTAGO_ATRAS: retorna la posición del vástago
...
void giro_lanzador(){
	...
	if (flag_sw2_giro){						//Condicional para el cambio de dirección de M2
		flag_sw2_giro = 0;					//Bandera de la interrupción SW2 a 0
		antirrebotes_sw2_giro();			//Si se detecta una pulsación en SW2, filtro el posible rebote
		if (flag_sw2_debounced)				//En caso de que la pulsación "sea buena"
		{
			flag_sw2_debounced = 0;			//Bandera del filtro de nuevo a 0
...
//LIGHT_LED: función para apagar y encender el LED
...
//GET_DISPARO: retorna cuando se ha producido el disparo
int get_disparo(){
    return flag_disparo;
}